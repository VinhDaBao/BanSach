...
public class AdminSachController {
	...
	public AdminSachController(SachService sachService, NhaCungCapService nccService, NhaXBService nxbService,
			HinhThucService htService, TheLoaiService theLoaiService) {
	...
	// 📘 Danh sách sách
	@GetMapping
	public String listSach(Model model, @RequestParam(defaultValue = "1") int page,
			@RequestParam(required = false) String keyword) {
	...
	@PostMapping("/save")
	public String save(@ModelAttribute("sach") Sach sach, @RequestParam(required = false) String hinhThucId,
			@RequestParam(required = false) String tenHTMoi, @RequestParam(required = false) List<Integer> theLoaiIds,
			@RequestParam(required = false) String tenTheLoaiMoi, @RequestParam(required = false) String nxbId,
			@RequestParam(value = "tenNXB", required = false) String tenNXB,
			@RequestParam(value = "diaChiNXB", required = false) String diaChiNXB,
			@RequestParam(value = "namThanhLapNXB", required = false) Integer namThanhLapNXB,
			@RequestParam("anhFile") MultipartFile anhFile, Model model) {
		try {
			if (sach.getId() != null) {
				Sach existing = sachService.findById(sach.getId()).orElse(null);
				if (existing != null) {
			...
			if (!anhFile.isEmpty()) {
				...
				String imageUrl = cloudinaryService.uploadFile(anhFile);
				sach.setAnh(imageUrl);
			} else if (sach.getId() != null) {
				// Nếu không upload mới, giữ lại ảnh cũ
				Sach existing = sachService.findById(sach.getId()).orElse(null);
				if (existing != null)
					sach.setAnh(existing.getAnh());
			}
			sachService.save(sach);
			return "redirect:/admin/sach";
		} catch (Exception e) {
